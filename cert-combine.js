...
function build(){const cert=$('certSection'),coa=$('coaSection'),notes=$('notes');if(!cert||!coa||!notes||$('certCombinedSection'))return;const wrap=document.createElement('div');wrap.id='certCombinedSection';wrap.className='wide card';wrap.style.margin='12px 0 0';wrap.innerHTML=`<h2>Certification / Certificate of Authenticity</h2><div class="grid"><label class="wide">Status<button type="button" id="certCombinedState" class="state" style="width:100%">❓ Unknown / Not Checked</button></label><div id="certCombinedDetails" class="wide"><div class="grid"><label>Certification Service<select id="certCombinedService"><option value="">None</option><option>PCGS</option><option>NGC</option><option>ICG</option><option>ANACS</option><option>Other</option></select></label><label>Certification Number<input id="certCombinedNumber"></label><label>Grade<input id="certCombinedGrade"></label><label>Certificate Issuer (if applicable)<input id="certCombinedIssuer"></label><label>Certificate Number (if applicable)<input id="certCombinedCoaNumber"></label><label class="wide">Scan / Verification Link<input id="certCombinedUrl" placeholder="https://..."><a id="certCombinedVerify" class="secondary" style="display:inline-block;margin-top:7px;padding:7px 10px;border-radius:8px;text-decoration:none;color:#111827" target="_blank" rel="noopener" hidden></a></label><label class="wide">Certificate Notes<textarea id="certCombinedCoaNotes"></textarea></label></div></div></div>`;notes.parentElement.insertBefore(wrap,notes);cert.classList.add('hidden-section');coa.classList.add('hidden-section');$('certCombinedState').onclick=cycle;$('certCombinedService').onchange=()=>{populateVerify();syncLegacy()};['certCombinedNumber','certCombinedGrade','certCombinedIssuer','certCombinedCoaNumber','certCombinedCoaNotes','certCombinedUrl'].forEach(id=>$(id).addEventListener('input',()=>{populateVerify();syncLegacy()}));const oldSave=$('saveBtn');if(oldSave)oldSave.addEventListener('click',syncLegacy,true);const oldClear=$('clearBtn');if(oldClear)oldClear.addEventListener('click',()=>setTimeout(loadFromLegacy,0),true);loadFromLegacy();removeLegacyCertificationSetting();}
function loadFromLegacy(){if(!$('certCombinedState')||!$('certService'))return;const z={service:$('certService').value||'',number:$('certNumber').value||'',grade:$('certGrade').value||'',url:$('certUrl').value||''};const c={issuer:$('coaIssuer').value||'',number:$('coaNumber').value||'',notes:$('coaNotes').value||'',status:+($('coaState').dataset.v||0)};$('certCombinedState').dataset.v=c.status===1?'1':c.status===2?'2':'0';$('certCombinedState').textContent=c.status===1?'✅ Yes':c.status===2?'❌ No':'❓ Unknown / Not Checked';$('certCombinedService').value=z.service;$('certCombinedNumber').value=z.number;$('certCombinedGrade').value=z.grade;$('certCombinedUrl').value=z.url;$('certCombinedIssuer').value=c.issuer;$('certCombinedCoaNumber').value=c.number;$('certCombinedCoaNotes').value=c.notes;updateDetails();populateVerify();}
function start(){build();removeLegacyCertificationSetting();setInterval(loadFromLegacy,500);setInterval(removeLegacyCertificationSetting,500);}
