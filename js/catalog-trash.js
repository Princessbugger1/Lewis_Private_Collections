...
function openTrash(){
  ...
  const restoreAll=rows.length?'<button type="button" class="secondary" id="ccTrashRestoreAll">Restore All</button>':'';
  modal.innerHTML='<div class="cc-trash-card" role="dialog" aria-modal="true"><div class="cc-trash-head"><div><h2>🗑️ Recycle Bin</h2><p>Deleted collection items stay here until you permanently remove them.</p></div><div class="cc-trash-head-actions">'+restoreAll+'<button type="button" class="secondary" id="ccTrashClose">Close</button></div></div><div id="ccTrashRows">'+(rows.length?rows.map((e,i)=>'<div class="cc-trash-row"><div><b>'+esc(label(e.item))+'</b><div class="small">Deleted '+esc(new Date(e.deletedAt||Date.now()).toLocaleString())+'</div></div><div class="cc-trash-actions"><button class="secondary" data-restore="'+i+'">Restore</button><button class="danger" data-purge="'+i+'">Delete permanently</button></div></div>').join(''):'<div class="empty">Trash is empty.</div>')+'</div></div>';
  document.body.appendChild(modal);$('ccTrashClose').onclick=()=>modal.remove();if($('ccTrashRestoreAll'))$('ccTrashRestoreAll').onclick=restoreAllItems;modal.onclick=e=>{if(e.target===modal)modal.remove()};
...
function restoreAllItems(){
  const trash=read(TRASH_KEY);if(!trash.length)return;
  const active=read(ITEM_KEY),kept=[],activeKeys=new Set(active.map(stableKey).filter(Boolean));let restored=0,skipped=0;
  trash.forEach(entry=>{const key=stableKey(entry?.item);if(key&&activeKeys.has(key)){kept.push(entry);skipped++;return}active.push(entry.item);if(key)activeKeys.add(key);restored++});
  if(!restored){alert('Nothing was restored because every item in Trash already has the same Catalog ID in the active collection.');return}
  try{saveActive(active);write(TRASH_KEY,kept)}catch(_){alert('Restore All was stopped because the collection could not be saved safely.');return}
  alert(restored+' item'+(restored===1?'':'s')+' restored.'+(skipped?' '+skipped+' duplicate Catalog ID'+(skipped===1?' was':'s were')+' left in Trash.':''));location.reload();
}
...
function styles(){if($('ccTrashStyle'))return;const s=document.createElement('style');s.id='ccTrashStyle';s.textContent='#ccTrashModal{position:fixed;inset:0;z-index:100200;background:#0008;display:grid;place-items:center;padding:16px}.cc-trash-card{width:min(680px,100%);max-height:90vh;overflow:auto;background:#fff;color:#111827;border-radius:14px;padding:16px;box-shadow:0 12px 40px #0005}.cc-trash-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.cc-trash-head-actions{display:flex;gap:7px;flex-wrap:wrap;justify-content:flex-end}.cc-trash-head h2{margin:0}.cc-trash-head p{font-size:13px;color:#6b7280;margin:4px 0 12px}.cc-trash-row{border:1px solid #e5e7eb;border-radius:10px;padding:11px;margin:8px 0;display:flex;gap:10px;justify-content:space-between;align-items:center}.cc-trash-actions{display:flex;gap:7px;flex-wrap:wrap}@media(max-width:520px){.cc-trash-head{display:block}.cc-trash-head-actions{justify-content:flex-start;margin-bottom:10px}.cc-trash-row{display:block}.cc-trash-actions{margin-top:9px}.cc-trash-actions button{width:100%}}';document.head.appendChild(s)}
