...
function patchSave(){const saveBtn=document.getElementById('saveBtn');if(!saveBtn||saveBtn.dataset.ccStatusWired)return;saveBtn.dataset.ccStatusWired='1';saveBtn.addEventListener('click',()=>{const vals=collect();setTimeout(()=>{try{const list=load();let idx=-1;try{idx=editing>=0?editing:list.length-1}catch(_){idx=list.length-1}if(idx<0||!list[idx])idx=list.length-1;if(idx>=0&&list[idx]){list[idx].checks={...(list[idx].checks||{}),...vals};localStorage.setItem(KEY,JSON.stringify(list));try{localStorage.setItem('lewis-private-collections-backup-v1',JSON.stringify(list))}catch(_){}try{if(typeof items!=='undefined'&&Array.isArray(items))items.splice(0,items.length,...list);if(typeof render==='function')render()}catch(_){}}}catch(_){}},20)},true)}
