...
function renderResearch(){const out=$('ccSavedResearchList');if(!out)return;const rows=read(RESEARCH_KEY),q=String($('ccSavedResearchSearch')?.value||'').trim().toLowerCase(),words=q.split(/\s+/).filter(Boolean);const shown=rows.filter(r=>words.every(w=>JSON.stringify(r).toLowerCase().includes(w)));$('ccSavedResearchCount').textContent=String(rows.length);if(!shown.length){out.innerHTML='<p class="small">'+(rows.length?'No matching saved research.':'No saved research yet.')+'</p>';return}out.innerHTML=shown.map(r=>`<article class="cc-saved-research-card" data-id="${esc(r.id)}"><strong>${esc(titleFrom(r).replace(/Untitled Draft/,'Untitled Research'))}</strong><p class="small">${esc([r.country,r.year,r.denomination,r.mint,r.type,r.variety].filter(Boolean).join(' • '))}</p><div class="actions"><button type="button" class="primary" data-open>Open / Continue Research</button><button type="button" data-add>Use for Add Item</button><button type="button" data-delete>Delete</button></div></article>`).join('');out.querySelectorAll('[data-open]').forEach(b=>b.onclick=()=>{const r=rows.find(x=>x.id===b.closest('[data-id]').dataset.id);if(r)loadResearch(r)});out.querySelectorAll('[data-add]').forEach(b=>b.onclick=()=>{const r=rows.find(x=>x.id===b.closest('[data-id]').dataset.id);if(!r)return;document.dispatchEvent(new CustomEvent('cc-identification-add-requested',{detail:{candidate:{country:r.country,type:r.type,denomination:r.denomination,year:r.year,mint:r.mint,variety:r.variety,title:titleFrom(r)},userApproved:true,referenceLink:r.referenceLink||''}}))});out.querySelectorAll('[data-delete]').forEach(b=>b.onclick=()=>{const id=b.closest('[data-id]').dataset.id;if(confirm('Delete this saved research?')){write(RESEARCH_KEY,rows.filter(x=>x.id!==id));renderResearch()}})}
function enhanceResearch(){const h=[...document.querySelectorAll('#ccResearchSection h3')].find(x=>x.textContent.trim()==='Research It Yourself');const block=h?.closest('.cc-research-block');if(!block)return;const actions=block.querySelector('.actions');if(actions&&!$('ccSaveResearch')){const b=document.createElement('button');b.type='button';b.id='ccSaveResearch';b.className='secondary';b.textContent='Save Research';b.onclick=saveResearch;actions.appendChild(b)}if(!$('ccSavedResearchDetails')){const d=document.createElement('details');d.id='ccSavedResearchDetails';d.className='cc-saved-research';d.innerHTML='<summary><b>Saved Research (<span id="ccSavedResearchCount">0</span>)</b></summary><input id="ccSavedResearchSearch" type="search" placeholder="Search country, type, year, mint, series, variety…" aria-label="Search saved research"><div id="ccSavedResearchList"></div>';block.insertAdjacentElement('beforebegin',d);$('ccSavedResearchSearch').addEventListener('input',renderResearch)}renderResearch()}
function addSaveDraftButton(){const save=$('saveBtn'),clear=$('clearBtn');if(!save||!clear)return;document.querySelectorAll('.actions button').forEach(b=>{if(b.id!=='ccSaveItemDraft'&&b.textContent.trim().toLowerCase()==='save as draft')b.remove()});let b=$('ccSaveItemDraft');if(!b){b=document.createElement('button');b.type='button';b.id='ccSaveItemDraft';b.className='secondary';b.textContent='Save as Draft';save.insertAdjacentElement('afterend',b)}b.onclick=saveItemDraft}
