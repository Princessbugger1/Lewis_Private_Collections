...
function buildFields(){const quantity=$('quantity'),composition=$('composition'),purchase=$('purchase'),value=$('value'),collection=$('collection'),location=$('location');if(!quantity||!composition||!purchase)return;
 ...
 const compositionLabel=composition.closest('label');
 const collectionLabel=collection?.closest('label');
 if(compositionLabel&&collectionLabel)compositionLabel.insertAdjacentElement('afterend',collectionLabel);
 if(!$('weightGroup')){const wrap=document.createElement('div');wrap.id='weightGroup';wrap.className='cc-extra-field cc-weight-group';wrap.innerHTML='<div class="cc-weight-title">Weight</div><div class="cc-weight-inner"><label>Actual Weight<input id="weight" inputmode="decimal" placeholder="e.g. 26.63 g"></label><label>Expected Weight<input id="expectedWeight" inputmode="decimal" placeholder="e.g. 26.73 g"></label></div>';collectionLabel?.insertAdjacentElement('afterend',wrap)}
 ...
 if(!$('ccExtraFieldStyle')){const st=document.createElement('style');st.id='ccExtraFieldStyle';st.textContent='.cc-extra-field{min-width:0}.cc-add-item-grid{align-items:start}.cc-add-item-grid>label,.cc-add-item-grid>.cc-extra-field{align-self:start;margin:0}.cc-weight-group{border:0;padding:0;align-self:start}.cc-weight-title{font-size:12px;font-weight:700;color:#4b5563;margin-bottom:4px}.cc-weight-inner{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.cc-weight-inner label{min-width:0}.cc-weight-inner input{min-width:0}.cc-value-field{align-self:start!important}@media(min-width:761px){.cc-add-item-grid #weightGroup{grid-column:1 / span 2}.cc-weight-group{margin-top:2px!important}.cc-weight-inner{grid-template-columns:repeat(2,minmax(140px,1fr))}}@media(max-width:420px){.cc-weight-inner{grid-template-columns:1fr}}';document.head.appendChild(st)}
 applyVisibility();
}
...
function loadAcquisition(){if(document.querySelector('script[data-cc-acquisition]'))return;const s=document.createElement('script');s.src='js/catalog-acquisition-fields.js?v=layout-20260913';s.dataset.ccAcquisition='1';document.head.appendChild(s)}
function init(){buildFields();addSettingsRows();bind();loadAcquisition();const observer=new MutationObserver(()=>{if($('settingsPanel')?.classList.contains('open'))addSettingsRows()});const p=$('settingsPanel');if(p)observer.observe(p,{attributes:true,attributeFilter:['class']})}
