...
function buildFields(){const quantity=$('quantity'),composition=$('composition'),purchase=$('purchase');if(!quantity||!composition||!purchase)return;
 ...
 if(!$('ccExtraFieldStyle')){const st=document.createElement('style');st.id='ccExtraFieldStyle';st.textContent='.cc-extra-field{min-width:0}.cc-add-item-grid{align-items:start}.cc-add-item-grid>label,.cc-add-item-grid>.cc-extra-field{align-self:start;margin:0}.cc-weight-group{border:1px solid #d1d5db;border-radius:9px;padding:7px 8px;align-self:start}.cc-field-title{display:block;font-size:12px;font-weight:650;color:#4b5563;margin-bottom:4px}.cc-weight-inner{display:grid;grid-template-columns:repeat(2,minmax(105px,1fr));gap:7px}.cc-weight-inner label{min-width:0}.cc-weight-inner input{min-width:0}@media(min-width:761px){.cc-add-item-grid #weightGroup{grid-column:5 / span 2}.cc-add-item-grid #weightGroup~label{align-self:start}.cc-add-item-grid #weightGroup+label{grid-column:1}.cc-weight-group{margin-top:0!important}}@media(max-width:420px){.cc-weight-inner{grid-template-columns:1fr}}';document.head.appendChild(st)}
 applyVisibility();
}
