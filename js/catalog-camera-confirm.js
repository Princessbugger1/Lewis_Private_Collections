/* Camera confirmation overlay for Add an Item scan: live camera -> capture -> Use This Picture / Retake / one Cancel-Close. */
(function(){
'use strict';
let active=false;
async function openCamera(side){
  ...
  modal.innerHTML=`<div class="cc-camera-card"><h3>${side==='Obverse'?'Obverse / Face':'Reverse'} Picture</h3><div class="cc-camera-frame"><video autoplay playsinline></video><img data-review hidden alt="Picture preview"><canvas hidden></canvas></div><div class="actions" data-live><button type="button" class="primary" data-snap>Take Picture</button></div><div class="actions" data-confirm hidden><button type="button" class="primary" data-use>Use This Picture</button><button type="button" class="secondary" data-retake>Retake</button></div><div class="actions cc-camera-close-row"><button type="button" class="secondary" data-close>Cancel / Close</button></div><p class="small" data-help>Take the picture, then use it, retake it, or cancel and close.</p></div>`;
...
.cc-camera-confirm-modal .actions{background:#fff!important;padding:10px 0 4px!important;margin-top:8px!important;z-index:2!important;display:flex!important;gap:8px!important;flex-wrap:wrap!important}
.cc-camera-confirm-modal .actions button{min-width:120px!important;flex:1 1 120px!important}
.cc-camera-confirm-modal [data-snap],.cc-camera-confirm-modal [data-use]{order:1}.cc-camera-confirm-modal [data-retake]{order:2}
.cc-camera-confirm-modal .cc-camera-close-row{position:sticky!important;bottom:0!important;padding-top:4px!important;margin-top:0!important}
