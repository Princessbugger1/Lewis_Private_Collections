...
async function openCamera(side){
  ...
  modal.innerHTML=`<div class="cc-camera-card"><h3>${side==='Obverse'?'Obverse / Face':'Reverse'} Picture</h3><div class="cc-camera-frame"><video autoplay playsinline></video><img data-review hidden alt="Picture preview"><canvas hidden></canvas></div><div class="actions cc-camera-main-actions"><button type="button" class="primary" data-snap>Take Picture</button><button type="button" class="primary" data-use disabled>Use This Picture</button><button type="button" class="secondary" data-retake disabled>Retake</button></div><div class="cc-camera-close-line"><button type="button" class="secondary compact" data-close>Cancel / Close</button></div></div>`;
