...
function styles(){if(document.getElementById('ccNavStyle'))return;const s=document.createElement('style');s.id='ccNavStyle';s.textContent='.cc-nav-bar{position:fixed;left:50%;bottom:12px;transform:translateX(-50%);z-index:100010;display:flex;gap:8px;align-items:center;padding:7px;background:#fffffff2;border:1px solid #d1d5db;border-radius:13px;box-shadow:0 5px 22px #0003;backdrop-filter:blur(8px)}.cc-nav-bar button{white-space:nowrap}.cc-nav-bar button:disabled{opacity:.45;cursor:default}body{padding-bottom:110px!important}@media(max-width:520px){.cc-nav-bar{bottom:8px;width:calc(100% - 16px);justify-content:center}.cc-nav-bar button{flex:1;max-width:160px}body{padding-bottom:calc(120px + env(safe-area-inset-bottom,0px))!important}}';document.head.appendChild(s)}
