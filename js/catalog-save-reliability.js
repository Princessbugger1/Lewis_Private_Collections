/* Lewis Private Collections — save feedback and post-save routing only.
...
function routeAfterCollectionSave(before,wasEdit){let tries=0;const check=()=>{if(snapshot()!==before){toast(wasEdit?'Changes saved':'Item saved');document.dispatchEvent(new CustomEvent('cc-open-section',{detail:{type:'collection'}}));return}if(++tries<30)setTimeout(check,50)};setTimeout(check,0)}
function installSaveFeedback(){const btn=$('saveBtn');if(!btn||btn.dataset.ccSaveFeedback)return;btn.dataset.ccSaveFeedback='1';btn.addEventListener('click',()=>{const before=snapshot();const wasEdit=(()=>{try{return typeof editing==='number'&&editing>=0}catch(_){return false}})();routeAfterCollectionSave(before,wasEdit)},true)}
