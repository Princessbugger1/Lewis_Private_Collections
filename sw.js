const CACHE='lewis-private-collections-v17';
...
self.addEventListener('fetch',event=>event.respondWith((async()=>{
 ...
 if(req.mode==='navigate'||req.destination==='document'){
  ...
  if(text.includes('</body>')&&!text.includes('back-navigation.js')){
   const injected='<script src="./back-navigation.js"></script>';
   const body=text.replace('</body>',injected+'</body>');
